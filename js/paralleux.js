...
(function adjustBarWidths() {
  window.addEventListener("resize", () => {  
    for(let i = 0; i < DOM_ELEMENTS.barDivs.length; i++) {
      DOM_ELEMENTS.barDivs[i].style.width = `${xTicks[parseFloat(DOM_ELEMENTS.barDivs[i].getAttribute("bar-value")) * 2].getBoundingClientRect().left - xTicks[0].getBoundingClientRect().left}px`;
      ...
      if(i < 9)
        DOM_ELEMENTS.barDivs[i].style.setProperty("--bar-index-left", `${-(xTicks[0].offsetLeft + 5)}px`);
      if(i >= 9)
        DOM_ELEMENTS.barDivs[i].style.setProperty("--bar-index-left", `${-(xTicks[0].offsetLeft + 3)}px`);
    }
  });
})();
...
export function yAxisBarIndxAndVal() {
  for(let i = 0; i < DOM_ELEMENTS.barDivs.length; i++) {
    const barValue = parseFloat(DOM_ELEMENTS.barDivs[i].getAttribute("bar-value"));
